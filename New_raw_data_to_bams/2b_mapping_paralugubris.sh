#!/bin/bash -l
#SBATCH -J map_paralugubris
#SBATCH -o /scratch/project_2001443/paralugubris/bam/logs/map_%j.out
#SBATCH -e /scratch/project_2001443/paralugubris/bam/logs/map_%j.err
...
# Set work directory
cd /scratch/project_2001443/paralugubris/
...
cd /scratch/project_2001443/paralugubris/bam/stats

for i in *_insert_size_metrics.txt
 do echo $i ; awk 'FNR == 8 {print $6}' $i
done > paralugubris.mean_insert_sizes.tmp

grep -v 'txt' paralugubris.mean_insert_sizes.tmp > tmp1
grep 'txt' paralugubris.mean_insert_sizes.tmp | perl -npe 's/_insert_size_metrics.txt//' > tmp2
paste tmp2 tmp1 > paralugubris.mean_insert_sizes.txt ; rm *tmp*

scp satokan1@puhti.csc.fi:'/scratch/project_2001443/paralugubris/bam/stats/paralugubris.mean_insert_sizes.txt' \
...
cd /scratch/project_2001443/paralugubris/bam/nodupl/stats

for i in *_nodupl.flagstat
 do echo $i ; awk 'FNR == 7' $i
done > paralugubris.mapping_rates.tmp

grep -v 'flagstat' paralugubris.mapping_rates.tmp > tmp1
grep 'flagstat' paralugubris.mapping_rates.tmp | perl -npe 's/_nodupl.flagstat//' > tmp2
paste tmp2 tmp1 > paralugubris.mapping_rates.txt ; rm *tmp*

scp satokan1@puhti.csc.fi:'/scratch/project_2001443/paralugubris/bam/nodupl/stats/paralugubris.mapping_rates.txt' \
'/Users/inaukkar/Library/CloudStorage/OneDrive-UniversityofHelsinki/PhD/4_formica_local_genomics/mapping_and_insert_stats/'
